...
7. **Relatório de validação de copy** — tabela por mensagem com os 5 gates (G1–G5) de `pipeline-validacao-copy.md` e ajustes feitos; variantes A/B dos toques de maior alavancagem com hipótese de cada uma.
8. **Racional estratégico + hipóteses** — por que cada decisão foi tomada; aprendizados anteriores aplicados (citar `learnings/`); hipóteses a testar em A/B.
